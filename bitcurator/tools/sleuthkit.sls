sleuthkit:
  cmd.run:
    - name: |
        cd /usr/share
        wget -q https://github.com/sleuthkit/sleuthkit/releases/download/sleuthkit-4.10.2/sleuthkit-4.10.2.tar.gz
        echo "Got sleuthkit 4.10.2 with wget" >> /var/log/bitcurator-install.log 2>&1
        rm -rf sleuthkit && tar zxf sleuthkit-4.10.2.tar.gz && mv sleuthkit-4.10.2 sleuthkit
        cd sleuthkit
