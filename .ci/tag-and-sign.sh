...
RELEASE_ID=`curl -XPOST -H "Authorization: token ${GITHUB_ACCESS_TOKEN}" -q https://api.github.com/repos/digitalsleuth/bitcurator-distro-salt/releases -d "{\"tag_name\": \"$TAG_NAME\", \"prerelease\": $PRERELEASE}" | jq .id`

echo "==> Downloading tar.gz file for tag from GitHub"
curl -qL -o /tmp/bitcurator-salt-states-${TAG_NAME}.tar.gz https://github.com/digitalsleuth/bitcurator-distro-salt/archive/$TAG_NAME.tar.gz
...
curl -XPOST -H "Authorization: token ${GITHUB_ACCESS_TOKEN}" -H "Content-Type: text/plain" -q "https://uploads.github.com/repos/digitalsleuth/bitcurator-distro-salt/releases/${RELEASE_ID}/assets?name=bitcurator-salt-states-${TAG_NAME}.tar.gz.sha256" --data-binary @/tmp/bitcurator-salt-states-$TAG_NAME.tar.gz.sha256

echo "==> Uploading bitcurator-salt-states-$TAG_NAME.tar.gz.sha256.asc"
curl -XPOST -H "Authorization: token ${GITHUB_ACCESS_TOKEN}" -H "Content-Type: text/plain" -q "https://uploads.github.com/repos/digitalsleuth/bitcurator-distro-salt/releases/${RELEASE_ID}/assets?name=bitcurator-salt-states-${TAG_NAME}.tar.gz.sha256.asc" --data-binary @/tmp/bitcurator-salt-states-$TAG_NAME.tar.gz.sha256.asc

echo "==> Uploading bitcurator-salt-states-$TAG_NAME.tar.gz.asc"
curl -XPOST -H "Authorization: token ${GITHUB_ACCESS_TOKEN}" -H "Content-Type: text/plain" -q "https://uploads.github.com/repos/digitalsleuth/bitcurator-distro-salt/releases/${RELEASE_ID}/assets?name=bitcurator-salt-states-${TAG_NAME}.tar.gz.asc" --data-binary @/tmp/bitcurator-salt-states-$TAG_NAME.tar.gz.asc
